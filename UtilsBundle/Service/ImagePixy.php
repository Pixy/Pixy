...
class ImagePixy {
   ...
   */
  public function Image ($src_or_resource=NULL)
  {
   ...
   */
  public function rotate ($degrees, $bkg='0')
  {
  ...
  /***
   * Rotate image right
   * Shortcut public function that will rotate the image to the right
   */
  public function rotate_right() { $this->rotate(270); }

  /***
   * Rotate image 180 degrees
   * Shortcut function that will rotate the image 180 degrees
   */
  public function rotate_180() { $this->rotate(180); }

  /***
   * Rotate image left
   * Shortcut function that will rotate the image to the left
   */
  public function rotate_left() { $this->rotate(90); }
   ...
   *
   */
  public function flip($bFlipH, $bFlipV=false)
  {
    $imgsrc = $this->res;
   ...
   */
  public function flip_h() { $this->flip(true, false); }
  public function flip_v() { $this->flip(false, true); }
  public function flip_both() { $this->flip(true, true); }
   ...
   */
  public function resize($newdim, $square=false, $bHeight=false, $resample=true) {
   ...
   */
  public function width($newdim, $square=false, $resample=true) { $this->resize($newdim, $square, false, $resample); }
  public function height($newdim, $square=false, $resample=true) { $this->resize($newdim, $square, true, $resample); }
  public function resize_1600($square=false) { $this->resize(1600, $square); }
  public function resize_1200($square=false) { $this->resize(1200, $square); }
  public function resize_1024($square=false) { $this->resize(1024, $square); }
  public function resize_800($square=false) { $this->resize(800, $square); }
  public function resize_640($square=false) { $this->resize(640, $square); }
   ...
   *		lots of conversions and quality isn't a huge issue
   */
  public function thumbnail($dest, $newdim, $square=false, $bHeight=false, $out=NULL, $resample=true) {
    $src_width 	= imagesx($this->res);
    $src_height = imagesy($this->res);
    $src_w 		= $src_width;
    $src_h 		= $src_height;
   ...
   */
  public function thumbnail_xsmall($dest, $out, $square=false) { $this->thumbnail($dest, $out, 60, $square); }
  public function thumbnail_small($dest, $out, $square=false) { $this->thumbnail($dest, $out, 80, $square); }
  public function thumbnail_medium($dest, $out, $square=false) { $this->thumbnail($dest, $out, 160, $square); }
  public function thumbnail_large($dest, $out, $square=false) { $this->thumbnail($dest, $out, 300, $square); }
  public function thumbnail_xlarge($dest, $out, $square=false) { $this->thumbnail($dest, $out, 512, $square); }
   ...
   */
  public function crop_top($px) { $this->crop($px); }
  public function crop_right($px) { $this->crop(0, $px, 0, 0); }
  public function crop_bottom($px) { $this->crop(0, 0, $px, 0); }
  public function crop_left($px) { $this->crop(0, 0, 0, $px); }
  public function crop_h($px) { $this->crop($px, 0, $px, 0); }
  public function crop_v($px) { $this->crop(0, $px, 0, $px); }
  public function crop_all($px) { $this->crop($px, $px, $px, $px); }
